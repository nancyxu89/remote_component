...
<template>
  <div class="ide-edit-page" ref="ideEditPage">
    ...
    <div class="action user-select clearfix" v-if="sqlData.length > 0">
        ...
        <div class="opt-right">
            <span class="action-format pdlr-0" title="格式化代码" @click="sqlFormat"><img src="src/assets/image/format.png"><span class="fs-14 mgl-4">格式化</span></span>
            <span class="action-change pdlr-0" @click="dialogThemeVisible=true"><i class="fa fa-eyedropper mgl-4" title="代码主题"></i><span class="fs-14">主题</span></span>
            <span class="action-change pdlr-0" @click="addDes"><i class="fa fa-code mgl-4" title="插入注释"></i><span class="fs-14">注释</span></span>
            <span class="action-change pdlr-0" @click="changeFontSize"><i class="fa fa-font mgl-4" title="字体大小切换"></i><span class="fs-14">字体</span></span>
            <span class="action-alt pdlr-0" @click="toggleEditorWindow"><i :class="['fa mgl-4',!isEnLarge?'fa-expand':'fa-compress']"
               title="代码区域缩展"></i><span class="fs-14">窗口</span></span>
      ...
      <el-dialog title="代码编辑器主题" :visible.sync="dialogThemeVisible">
          <el-form label-width="110px">
              <el-form-item label="主题名：">
                  <el-select v-model="aceTheme" placeholder="" size="small">
    ...
    methods: {
      ...
      setCurQueryResult (idTimeStamp, isNeedAssign = false) {
          ...
          if (oldTabId && (oldTabId !== (this.selectTabId + ''))) {
              ...
              // 设置简单的当前选中结果项
              let params = ['columnNames', 'engine', 'recordNum', 'searchValue', 'queryId', 'idTimeStamp']
      ...
      sortData (allData) {
          ...
          if (sortOrder && (sortOrder === 'descending' || sortOrder === 'ascending')) {
              let prop = this.resultSortObj[this.selectedResultId].prop
              let arr = JSON.parse(JSON.stringify(allData))
              let isDes = sortOrder === 'descending'
              arr.sort((a, b) => {
                  let aa = (a[prop] + '') === 'NULL' ? '' : (a[prop] + '')
                  let bb = (b[prop] + '') === 'NULL' ? '' : (b[prop] + '')
      ...
      handleTableSort (column) {
          ...
          this.filterTableData(this.resTableData[this.curQueryResult.index].currentPage)
